...
def get_current_city_weather_data(query_str):
    """Retrieve the current weather data from a query string"""
    result = dict()
...
def get_five_days_forecast(query_str):
    """Retrieve the five days weather data and make a line chart based on the data."""
    result = {}
    response = request('GET', query_str)
    data = response.json()
